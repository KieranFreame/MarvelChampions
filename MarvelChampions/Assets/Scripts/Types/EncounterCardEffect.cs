...
public class EncounterCardEffect : ScriptableObject, IEffect
{
    ...
    public virtual async Task OnEnterPlay() { await EffectManager.Inst.AddEffect(_card, this); }
    public virtual async Task OnExitPlay() { await Task.Yield(); }
    public virtual async Task WhenRevealed(Villain owner, EncounterCard card, Player player) { await OnEnterPlay(); }
